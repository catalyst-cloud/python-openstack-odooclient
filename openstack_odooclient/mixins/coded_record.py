...
class CodedRecordMixin(RecordProtocol[RM], Generic[RM]):
    """A record mixin for record types with a ``code`` field.

    Include this mixin to add the ``code`` field, with ``str`` type,
    to your custom record class. This allows the additional methods
    provided by ``CodedRecordManagerMixin`` to be used.
    """
...
class CodedRecordManagerMixin(RecordManagerProtocol[R], Generic[R]):
    """A record manager mixin for record types with a ``code`` field.

    The ``code`` field is expected to be unique, which allows
    methods for getting records by code to be defined.

    The record class must either include the ``CodedRecordMixin`` mixin,
    or have its own ``code`` field defined with the ``str`` type.
    """
